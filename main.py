...
class AuditApp:
    ...
    def _auto_fix_issue(self) -> None:
        ...
        linux_pages = {
            ...
            "UFW Firewall Enabled": "/etc/default",
            "Unattended Upgrades Enabled": "/etc/apt/apt.conf.d",
            "ASLR Enabled": "/etc/sysctl.conf",
            "AppArmor Enabled": "/etc/apparmor.d",
            "No Unauthorized UID 0 Accounts": "/etc/passwd",
            "Secure Umask Configuration": "/etc/login.defs",
            "Core Dumps Restricted": "/etc/security/limits.conf",
            "Auditd Service Running": "/etc/audit",
            "Sticky Bit on /tmp": "/tmp",
            "Sudo Requires Password": "/etc/sudoers.d",
            "SMB Server Min Protocol": "/etc/samba",
            "Default Admin Account Avoided": "/etc/passwd",
        }
